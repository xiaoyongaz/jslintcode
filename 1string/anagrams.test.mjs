...
import {anagrams} from "./anagrams.mjs";
/*
Example 1:
...
test('[lint, intl, inlt, code] should return [lint, inlt, intl]', t => {
    let result = anagrams(['lint', 'intl', 'inlt', 'code']);
    console.log(`result is ${result}`);
    assert.deepEqual(result,['lint', 'intl', 'inlt'], "should return  ['lint', 'intl', 'inlt']");
});

test('["ab", "ba", "cd", "dc", "e"] should return ["ab", "ba", "cd", "dc"]', t => {
    let result = anagrams(["ab", "ba", "cd", "dc", "e"]);
    console.log(`result is ${result}`);
    assert.deepEqual(result,["ab", "ba", "cd", "dc"], "should return ['ab', 'ba', 'cd', 'dc'] ");
});
